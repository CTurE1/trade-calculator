...
with st.container():
    ...
    if old_price > 0:
        ...
        # ── с комиссией 5 % ──
        fee_percent = 5.0  # если нужна другая – поменяйте тут
...
with st.container():
    st.markdown('<div class="card">', unsafe_allow_html=True)
    st.markdown(
        '<div class="title">🔄 Конвертация прокси под MarketApp</div>',
        unsafe_allow_html=True,
    )
    ...
    if proxy_input:
        ...
        if converted:
            ...
            st.code(converted, language="text")  # ← одна кнопка-иконка «Copy»
        else:
            st.warning("❌ Неверный формат. Требуется: IP:PORT:USER:PASS")

    # Поле только для чтения (без копирки) — удобно выделить/править вручную
    st.text_area(
